...
import { txs } from '@blockfrost/blockfrost-js/lib/endpoints/api/txs';
...
export function mergeTxAndWitness(tx:Transaction,walletWitnesses: TransactionWitnessSet):HexString{
  ...
  console.log("OLD_TX: " , oldTX.to_js_value());
  console.log("Transaction: " , tx.to_js_value());
...
export  function parseCardanoTransaction(_tx: string) : Transaction{
  ...
  try {
      ...
      console.log("parsedCardanoTransaction (pre) :" , tx.to_js_value() );
    ...
    console.log("parsed cardano transaction (post): ", transaction.to_js_value());
